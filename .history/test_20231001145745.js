...
const nextDecision = (states, state) => {
  let bestScore = queryStateScore(states, state),
    bestAction = [true, true, true, true, true];

  const generateMaskScore = (sts, st, maskNum, step, action) => {
    if (step > 5 && maskNum == 0) {
    ...
    if (step <= 5 && maskNum >= 0) {
      generateMaskScore(sts, st, maskNum, step + 1, [...action, true]);
      generateMaskScore(sts, st, maskNum - 1, step + 1, [...action, false]);
    }
  };
  for (let maskNum = 1; maskNum > 0; maskNum--) {
    generateMaskScore(states, state, maskNum, 1, []);
  }
  return { bestAction, bestScore };
};

const space = getStateSpace();
// console.log(nextDecision(space, [1, 1, 1, 1, 1]));
console.log(
  calScoreRecursively(space, [1, 1, 1, 1, 1], 5, [true, true, true, true, true])
);
