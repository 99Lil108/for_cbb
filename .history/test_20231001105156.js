...
const generateMaskScore = (states, state, maskNum, step, action) => {
  if (step > 5 && maskNum == 0) {
    let [tempS, _] = calScoreRecursively(states, state, 5, action);
  }
  if (step <= 5 && maskNum >= 0) {
    generateMaskScore(maskNum, step + 1, [...action, true]);
    generateMaskScore(maskNum - 1, step + 1, [...action, false]);
  }
};
