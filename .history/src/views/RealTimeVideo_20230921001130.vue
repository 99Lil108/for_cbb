...
let socket = null;
function applyV() {
  if (socket) socket.emit("apply", "v_dljsjc");
  restart.value = false;
}
onMounted(() => {
  ...
  socket.on("dataComing", (base64Image) => {
    if (base64Image == "@") restart.value = true;
    else imageElement.value.src = `data:image/png;base64,${base64Image}`;
  });
});
onUnmounted(() => {
